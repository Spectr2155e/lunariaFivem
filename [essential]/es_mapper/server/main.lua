...
function LoadMap(name)
	...
	local xml = LoadResourceFile('es_mapper', 'data/maps/' .. name .. '.xml')
...
function SaveMap(name, objects)
	...
	xml = xml .. '  <Metadata>'                                                                                             .. "\n"
	xml = xml .. '    <Creator>es_mapper</Creator>'                                                                         .. "\n"
	xml = xml .. '    <Name>es_mapper</Name>'                                                                               .. "\n"
	xml = xml .. '    <Description>es_mapper</Description>'                                                                 .. "\n"
	...
	file = io.open('resources/[esx]/[essential]/es_mapper/data/maps/' .. name .. '.xml', 'w')
...
RegisterServerEvent('es_mapper:beginSaveObjects')
AddEventHandler('es_mapper:beginSaveObjects', function(name)
	local _source = source
	ObjectsToSave[source .. '_' .. name] = {}
end)

RegisterServerEvent('es_mapper:endSaveObjects')
AddEventHandler('es_mapper:endSaveObjects', function(name)
	local _source = source
	SaveMap(name, ObjectsToSave[source .. '_' .. name])
	TriggerClientEvent('es_mapper:showNotification', _source, 'Map [' .. name .. '] saved')
end)


RegisterServerEvent('es_mapper:addObjectToSave')
AddEventHandler('es_mapper:addObjectToSave', function(name, object)
	local _source = source
	table.insert(ObjectsToSave[source .. '_' .. name], object)
end)

RegisterNetEvent('es_mapper:loadObjects')
AddEventHandler('es_mapper:loadObjects', function(name)
	
	local _source = source
	local dom     = LoadMap(name)

	if dom == nil then
		TriggerClientEvent('es_mapper:showNotification', _source, 'Map [' .. name .. '] does not exists')
	else
		...
		CreateThread(function()
			...
			for i=1, #objects, 1 do
				...
				TriggerClientEvent('es_mapper:loadObject', _source, objects[i])
			...
			TriggerClientEvent('es_mapper:showNotification', _source, 'Map [' .. name .. '] loaded')
